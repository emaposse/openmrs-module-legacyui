				...
				<tbody id="identifiersTbody">
					...
					<td class="idNumberDataColumn" valign="top">
						...
						<div id="initialLocationBox${varStatus.index}" style="${(behavior == 'NOT_USED' || empty id.identifierType) ? 'display:none;' : ''}">
							<!-- Using spring:bind because Form:options caused: https://issues.openmrs.org/browse/LUI-32 -->
							<spring:bind path="location">
								<select id="locationId" name="${status.expression}">
									<option></option>
									<c:forEach items="${locations}" var="loc">
										<option value="${loc.locationId}" <c:if test="${id.location.locationId == loc.locationId}"> selected="selected"</c:if>><c:out value="${loc.name}"/></option>
									</c:forEach> 
								</select>			
							</spring:bind>
						</div>
